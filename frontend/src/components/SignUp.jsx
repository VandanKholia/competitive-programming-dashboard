...
function SignUp() {
  ...
  const handleSignUp = (e) => {
    ...
    api.post('/api/auth/signup', { name, email, password })
      .then(result => {
        // After successful signup, verify that httpOnly cookies are being set
        api.get('/api/auth/user')
          ...
          .catch((verifyErr) => {
            ...
            if (verifyErr.response && verifyErr.response.status === 401) {
              alert(
                "Account created successfully, but third-party cookies appear to be disabled in your browser.\n\n" +
                "Please enable third-party cookies in your browser settings to use this app."
              );
              setPassword("");
            } else {
              console.error('Verification error:', verifyErr);
              alert("Account created but session verification failed. Please try logging in.");
              navigate("/login");
            }
            setIsLoading(false);
          });
          ...
          <form onSubmit={handleSignUp} className="space-y-6">
            ...
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-3 border-2 rounded-xl focus:ring-4 outline-none transition-all duration-200 text-gray-900 placeholder-gray-500 ${error ? "border-red-500 focus:ring-red-200" : "border-gray-200"
                  }`}
